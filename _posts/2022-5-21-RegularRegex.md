---
layout: post
title: Qt有关正则表达式的写法与应用实例以及常见的正则匹配模式
description: 一些常见的正则匹配的实例，比如匹配IP等等，各种匹配格式
categories: 实例
keywords: 正则,Qt,实例,经验之谈
---
...
然后首先要知道正则表达式怎么写，这个具体我就不在这里详谈了，详情请参考[ 正则表达式-菜鸟教程 ](https://www.runoob.com/regexp/regexp-tutorial.html)
...
我们都知道在传统的正则表达式中我们会用.* 来表示任意字符，但是在qt的正则模式中并不是这样的，它有自己的规则，比如用\S来表示匹配任意非空白的字符，用\d来匹配任意数字，而没有.的概念，具体参数表见[Qt——正则表达式](https://www.cnblogs.com/hellovenus/p/4984751.html)，这里面的用法其实写的很详尽了。
