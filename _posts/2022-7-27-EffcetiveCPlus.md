...
## 16.new 和 delete 保持相同的格式
 
如果在new 表达式中使用[]，则必须在相应的delete表达式中也使用[]。反之则不 
 
## 17.以独立语句将newed对象置入智能指针
 
以独立语句将newed对象存储于智能指针内。如果不这样做，一旦异常被抛出，有可能导致难以察觉的资源泄露。
 
 不懂，看不太明白，之后再注释吧，多看多想。
 
## 18.让接口容易被正确使用，不容易被误用
 
 1.好的接口很容易被正确使用，不容易被无用。你应该在你的所有接口中努力达成这些性质
 
 2.促进正确使用 的办法包括接口的一致性以及与内置类型的行为兼容。
 
 3.阻止误用的办法包括建立新类型、限制类型上的操作，束缚对象值，以及消除客户的资源管理责任
 
 4.tr1::shared_ptr支持定制性删除器。这可防范DLL问题，可被用来自动解除互斥锁等等
 
 ## 19.设计class犹如设计type
 
 设计一个class都应该要面对以下几个问题，这几个问题会导致设计规范的水平参差不齐
 
 1.新type的对象应该如何被创建和销毁？
 
 这会影响到构造函数和析构函数、内存分配函数和释放函数的设计，当然前提是我还打算写，只能说编译器会帮我补上
 
 2.对象的初始化和对象的赋值该有什么区别？
 
 这个将决定构造函数和赋值操作符的行为，初始化 和 赋值 不是一码事。一个是分配初始内存，一个是让这个对象 等于另外一个对象
 
 3.新type的对象如果被passed by value 以值传递，意味着什么？
 
 这将决定你该如何定义一个copy构造函数
 
 4.什么是新type的“合法值”？
 
 对于class的成员变量而言，通常只有某些数值集是有效的。那些数值集决定了你的class必须维护的约束条件，也就决定了你的成员函数必须进行错误检查工作。它也会影响函数抛出的异常。
 
 5.新的type需要配合某个继承来做吗？
 
 如果允许其他的类来继承这个类，这就会影响到声明的函数->尤其是析构函数 是否为virtual ，详情见no.7
 
 6.你的新type需要什么样的转换？
 
 你的type身存与一大票type之间，彼此之间会有转换行为吗？比如QGeometry QSize QPoint 之类的，这种各种乱七八糟的类型之间的转换，是什么样的？可能需要提出一些转换函数。
 
 7.什么样的操作对于这个类是合理的？
 
 这将决定你要声明哪些函数，哪些是成员函数，公共函数，私有函数，变量等。
 
 8.什么样的标准函数应该驳回？
 
 把他们声明为private，大概就是哪些不让客户去操作的中间函数什么的，反正就是不想让你碰，别来挨边。
 
 9.谁该取用新type的成员？
 
 这将决定哪个成员要成为public，哪个是protect，哪个是private，总之不是你想让他public就public的，客户有时候就这么傻逼，不爽不要写。
 
 10.你的新type是一般化的吗？
 
 其中的内容就是个别参数不同？或者说你不是定义了一个type，而是个types？老哥，你该写个class template了
 
 11.你真的需要一个新type吗？
 
 讲道理，新分出来一个class，真不一定好吧，老轮子能用就用，加个函数又不是不能用，对吧。
