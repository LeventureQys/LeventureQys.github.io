...
RVC 的语音模型文件（比如用户训练出来的 `xxx_e200_s8000.pth`）并不是只存了声码器的权重——它存的是**整个合成器**的完整快照。用 `torch.load()` 加载后，得到的是一个字典，结构如下：

```python
cpt = torch.load("model.pth", map_location="cpu")

cpt["weight"]    # 模型权重（state_dict），包含所有子模块的参数
cpt["config"]    # 模型结构配置，一个列表，18 个元素
cpt["f0"]        # 0 或 1，表示该模型是否使用 F0
cpt["version"]   # "v1" 或 "v2"
cpt["info"]      # 训练信息字符串（比如 epoch 数）
```

这里的 `cpt["weight"]` 是一个扁平化的字典，key 是参数的完整路径名。对于一个使用 F0 的 v1 模型，里面包含了这些子模块的全部参数：

```
enc_p.emb_phone.weight          ← TextEncoder 的 phone 嵌入层
enc_p.emb_pitch.weight          ← TextEncoder 的 pitch 嵌入层
enc_p.encoder.*                 ← TextEncoder 的 Transformer 编码器
enc_p.proj.*                    ← TextEncoder 的投影层
dec.conv_pre.*                  ← GeneratorNSF 的预卷积
dec.ups.0.weight / .bias        ← GeneratorNSF 第 0 级上采样转置卷积
dec.ups.1.weight / .bias        ← GeneratorNSF 第 1 级上采样转置卷积
...
dec.noise_convs.0.*             ← GeneratorNSF 第 0 级谐波注入卷积
dec.noise_convs.1.*             ← ...
...
dec.resblocks.0.convs1.*        ← GeneratorNSF 的 ResBlock 权重
dec.resblocks.0.convs2.*
...
dec.m_source.l_sin_gen.*        ← SourceModuleHnNSF（SineGen 本身无可学参数）
dec.m_source.l_linear.*         ← SourceModuleHnNSF 的谐波混合层
dec.conv_post.*                 ← GeneratorNSF 的后卷积
flow.flows.*                    ← ResidualCouplingBlock 的 flow 层
emb_g.weight                    ← 说话人嵌入表
```

注意 `dec.*` 开头的就是声码器 `GeneratorNSF` 的权重。声码器的上采样卷积、noise_convs、所有 ResBlock、SourceModule 的线性层——全部保存在这个 `.pth` 文件里。

`cpt["config"]` 是一个列表，按顺序存放合成器构造函数的参数：
...
加载过程在 `infer/modules/vc/modules.py:103-127` 的 `get_vc()` 方法中完成。整个过程分四步：

**第一步：读取文件，解析元信息**

```python
self.cpt = torch.load(person, map_location="cpu")
self.tgt_sr = self.cpt["config"][-1]                           # 采样率
self.cpt["config"][-3] = self.cpt["weight"]["emb_g.weight"].shape[0]  # 说话人数
self.if_f0 = self.cpt.get("f0", 1)                             # 是否用 F0
self.version = self.cpt.get("version", "v1")                   # 版本
```

这里有个细节：说话人数量（`spk_embed_dim`）不是从 config 里读的，而是直接看 `emb_g.weight` 这个嵌入表有多少行。这样即使 config 里的值不对也不影响。

**第二步：根据版本和 F0 选项，创建对应的合成器空壳**

```python
synthesizer_class = {
    ("v1", 1): SynthesizerTrnMs256NSFsid,       # v1 + F0 → 用 GeneratorNSF
    ("v1", 0): SynthesizerTrnMs256NSFsid_nono,   # v1 无 F0 → 用标准 Generator
    ("v2", 1): SynthesizerTrnMs768NSFsid,        # v2 + F0 → 用 GeneratorNSF
    ("v2", 0): SynthesizerTrnMs768NSFsid_nono,   # v2 无 F0 → 用标准 Generator
}
...
这一步用 config 列表里的参数构建了完整的合成器（包括 TextEncoder、GeneratorNSF、PosteriorEncoder、Flow、说话人嵌入）。此时所有权重还是随机值。

**第三步：删掉推理不需要的模块，加载权重**

```python
del self.net_g.enc_q                                     # 后验编码器只在训练时用
self.net_g.load_state_dict(self.cpt["weight"], strict=False)  # 加载权重
```

`strict=False` 是因为刚删了 `enc_q`，weight 字典里有 `enc_q.*` 的键但模型里已经没这个模块了，设 strict=False 就不会报错。
...
if self.config.is_half:
    self.net_g = self.net_g.half()     # FP16 推理
else:
    self.net_g = self.net_g.float()    # FP32 推理
```

到这里，声码器（作为 `self.net_g.dec` ）就带着训练好的权重准备就绪了。
...
```python
# pipeline.py:235-245
score, ix = index.search(npy, k=8)           # 用 HuBERT 特征去索引里查 8 个最近邻
weight = np.square(1 / score)                  # 距离越近权重越大
weight /= weight.sum(axis=1, keepdims=True)
npy = np.sum(big_npy[ix] * np.expand_dims(weight, axis=2), axis=1)  # 加权平均

feats = torch.from_numpy(npy) * index_rate + (1 - index_rate) * feats
#       └── 检索到的目标音色特征 ──┘            └── 原始 HuBERT 特征 ──┘
```

简单说就是：把输入音频的 HuBERT 特征拿去跟训练数据的特征库做检索，找到最像的几条目标说话人的特征，混合后替换掉原始特征。`index_rate` 控制替换的比例——设得越高，输出越像目标说话人，但可能丢失一些原始的语音细节。
